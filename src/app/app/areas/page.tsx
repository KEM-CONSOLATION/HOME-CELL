...
import { listAreasPage, deleteArea } from "@/lib/areas-api";
...
export default function AreasDirectoryPage() {
  ...
  useEffect(() => {
    void fetchAreas(page);
  }, [page]);

  const fetchAreas = async (currentPage: number) => {
    setIsLoading(true);
    try {
      const data = await listAreasPage(currentPage);
      setAreas(data.items);
