...
import { getState, deleteState } from "@/lib/states-api";
...
export default function StateDetailsPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const data = await getState(idNum);
        if (!cancelled) setState(data);
      } catch (error) {
        console.error("Failed to fetch state:", error);
        if (!cancelled) toast.error("Failed to load state");
      } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success("State removed", {
        description: `${state.name} was removed.`,
      });
  ...
  if (!state || !Number.isFinite(idNum)) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        <p className="text-muted-foreground font-bold">State not found</p>
        <Link
          href="/app/states"
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48">
            <DropdownMenuItem asChild>
              <Link href={`/app/states/${state.id}/edit`}>Edit state</Link>
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">State record</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card className="border-none bg-white">
          <CardHeader>
            ...
            <CardDescription>Identifiers from the API</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 text-sm">
            <div className="flex justify-between gap-4">
              <span className="text-muted-foreground">State ID</span>
              <span className="font-semibold">{state.id}</span>
            </div>
            <div className="flex justify-between gap-4">
              <span className="text-muted-foreground">State pastor</span>
              <span className="font-semibold">{state.state_pastor}</span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4 text-sm">
            <div className="flex justify-between gap-4">
              ...
              <span className="font-semibold">
                {dayjs(state.created_at).format("MMM D, YYYY h:mm A")}
              </span>
            </div>
            <Link
              href={`/app/states/${state.id}/edit`}
      ...
      <ConfirmDeleteModal
        ...
        description="Areas and other records tied to this state may be affected. This cannot be undone."
        itemName={state.name}
