...
export default function ZoneDetailsPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const data = await getZone(idNum);
        if (!cancelled) setZone(data);
      } catch (error) {
        console.error("Failed to fetch zone:", error);
        if (!cancelled) toast.error("Failed to load zone");
      } finally {
  ...
  if (!zone || !Number.isFinite(idNum)) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        <p className="text-muted-foreground font-bold">Zone not found</p>
        <Link
          href="/app/zones"
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48">
            <DropdownMenuItem asChild>
              <Link href={`/app/zones/${zone.id}/edit`}>Edit zone</Link>
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {zone.area_name} · {zone.state_name}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card className="border-none bg-white">
          ...
          <CardContent className="space-y-4 text-sm">
            ...
            <div className="flex justify-between gap-4">
              <span className="text-muted-foreground">Zonal leader</span>
              <span className="font-semibold">{zone.zonal_leader}</span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4 text-sm">
            <div className="flex justify-between gap-4">
              ...
              <span className="font-semibold">
                {dayjs(zone.created_at).format("MMM D, YYYY h:mm A")}
              </span>
            </div>
            <Link
              href={`/app/zones/${zone.id}/edit`}
